    ...
    def check(self, weijinFlag=True):
        ...
        if i != False:
            ...
            if weijinFlag == True and gid != None and self.data.se.get("sender").get("role") == "member":
                client.send('[CQ:face,id=151] [CQ:at,qq={2}] {0}不喜欢您使用（{1}）这种词语哦，请换一种表达方式吧！'.format(self.data.botSettings.get('name'), i, self.data.se.get("user_id"))).msg()
                self.utils.coin.rm()
