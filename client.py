...
import statement.FaceStatement
...
import random, math, mysql, requests, re
...
class Client:
    ...
    def CallApi(self, api, parms={}, timeout=10):
        ...
        if not botSettings:
            botSettings = mysql.selectx('SELECT * FROM `botBotconfig` WHERE `uuid`="{0}";'.format(self.data.uuid))[0]
