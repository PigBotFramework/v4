...
import requests
from ..model.BanWordsModel import BanWordsModel

class BanWords:
    data: Struct = None
    utils: Utils = None
    model: BanWordsModel = None

    def __init__(self, data: Struct):
        self.data = data
        self.utils = Utils(self.data)
        self.model = BanWordsModel()

    def find(self, message):
        for l in self.model._getAll():
            i = l.get('content')
    ...
    def check(self, weijinFlag=True):
        ...
        if i != False:
            ...
            if weijinFlag == True and gid != None and self.data.se.get("sender").get("role") == "member":
                print('wssb')
                client.msg().raw('[CQ:face,id=151] [CQ:at,qq={2}] {0}不喜欢您使用（{1}）这种词语哦，请换一种表达方式吧！'.format(self.data.botSettings._get('name'), i, self.data.se.get("user_id")))
                self.utils.coin.remove()
                client.CallApi('delete_msg', {'message_id':self.data.se.get('message_id')})
                
            # 如果辱骂机器人则骂回去
            if ('[CQ:at,qq='+str(self.data.botSettings._get('myselfqn'))+']' in messageReplace) or (self.data.botSettings._get('name') in messageReplace) or ('猪比' in messageReplace) or ('猪逼' in messageReplace) or ('猪鼻' in messageReplace) or ('机器人' in messageReplace) or (gid == None):
                repeatnum = self.data.botSettings._get('yiyan')
                while repeatnum > 0:
                    self.utils.coin.remove()
                    dataa = requests.get(url=self.data.botSettings._get('duiapi'))
                    dataa.enconding = "utf-8"
                    if repeatnum == self.data.botSettings._get('yiyan'):
