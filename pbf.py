...
import mysql, cache, sys, random, threading, importlib, traceback
...
class PBF:
    ...
    def reply(self):
        def replyFunc():
            try:
                # message = str(ChatterBot.get_response(self.data.message))
                message = 'abab'
                if message != "你是吹牛":
                    if '菲菲' in message:
                        message = message.replace('菲菲', self.data.botSettings.get("name"))
                    if random.randint(0,5) == 3:
                        if '{face:' in message:
                            message = message.replace('{face:', "表情").replace("}", " ")
                        self.client.msg(
                            Statement("tts", text=message)
                        ).send()
                    else:
                        if '{face:' in message:
